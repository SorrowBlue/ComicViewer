...
internal class ImageCacheDataSourceImplDagger @Inject constructor(
    private val lazyCoilDiskCache: dagger.Lazy<CoilDiskCache>,
    private val imageCacheDiskCache: dagger.Lazy<DiskCache>,
    ...
    override fun getBookshelfImageCacheInfo(bookshelf: Bookshelf): Resource<BookshelfImageCacheInfo, Resource.SystemError> {
        return kotlin.runCatching {
            val thumbnailImageCache = lazyCoilDiskCache.get().thumbnailDiskCache(bookshelf.id).let {
                ThumbnailImageCache(it.size, it.maxSize)
            }
            val bookPageImageCache = lazyCoilDiskCache.get().pageDiskCache(bookshelf.id).let {
    ...
    override suspend fun clearImageCache(
        ...
        return kotlin.runCatching {
            when (imageCache) {
                is BookPageImageCache -> lazyCoilDiskCache.get().pageDiskCache(bookshelfId).clear()
                is OtherImageCache -> imageCacheDiskCache.get().clear()
                is ThumbnailImageCache -> lazyCoilDiskCache.get().thumbnailDiskCache(bookshelfId)
                    .clear()
            }
        }.fold(
